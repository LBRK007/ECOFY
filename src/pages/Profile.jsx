...
function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchWishlistProducts = async () => {
      ...
      try {
        // Fetch each wishlisted product doc from Firestore
    ...
    >
      ...
      <div style={styles.tabRow}>
        ...
        >
          Wishlist 🤍
          {wishlist.length > 0 && (
            <span style={styles.tabBadge}>{wishlist.length}</span>
          )}
        </button>
      </div>
        ...
        <>
          {orders.length === 0 ? (
            ...
            orders.map((order) => {
              ...
              return (
                <motion.div key={order.id} layout initial={{ opacity: 0, y: 12 }}
                  animate={{ opacity: 1, y: 0 }} style={styles.orderCard}>
                  <div style={styles.orderTop}>
                    <div>
                      <p style={{ margin: "0 0 4px" }}><strong>Total:</strong> ₹ {order.total}</p>
                      ...
                      <motion.div key="confirm"
                        ...
                        <div style={styles.confirmBox}>
                          ...
                          <div style={{ display: "flex", gap: "10px" }}>
                            <button onClick={() => handleCancel(order)} style={styles.confirmYes}>Yes, cancel it</button>
                            <button onClick={() => setConfirmId(null)} style={styles.confirmNo}>Keep order</button>
                          </div>
                        </div>
                      </motion.div>
                    )}
                  ...
                  <div style={styles.shippingBox}>
                    <p style={{ margin: "0 0 4px", fontWeight: "500", fontSize: "13px" }}>Shipping address</p>
                  ...
                  >
                    ...
                    >
                      <img
                        src={product.image}
                        alt={product.name}
                        style={{ width: "100%", aspectRatio: "1", objectFit: "cover", borderRadius: "10px", display: "block" }}
                      />
